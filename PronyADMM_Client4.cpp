...
int main(int argc, char *argv[])
{
    ...
    int sockfd3 = 0;
   	...
   	if(inet_pton(AF_INET, argv[5], &serv_addr.sin_addr)<=0){
        	printf("\n inet_pton error occured\n");
        	return 1;
    	  } 
    ...
    int sockfd4 = 0;
    	memset(&serv_addr, '0', sizeof(serv_addr));   
    	serv_addr.sin_family = AF_INET;
    	serv_addr.sin_port = htons(atoi(argv[8])); 
	...
	if((sockfd4 = socket(AF_INET, SOCK_STREAM, 0)) < 0)   {
        ...
        if(connect(sockfd4, (struct sockaddr *)&serv_addr, sizeof(serv_addr)) == -1)   {
            perror("\n Error : Connect Failed \n");
            return 1;
        } else {
             printf("\n TCP Connection has been set up\n");
   ...
   while(Iteration < SamNum){
	...
	CountClient = 0;
		...
		for (i=0; i<4; i++){
        		...
        		if (i==3){
        			avgBufferLen = read(sockfd4, avgBuffer, DEFAULT_MAX_BUFFER_LEN);
					pht = strtok(avgBuffer, " ");
					pht = strtok(NULL, " ");
					pht = strtok(NULL, " ");
					pht = strtok(NULL, " ");					new_avgpara(i,0) = strtod(pht, NULL);
	...
	new_u(Iteration,0)=-(K(3,3)*new_avgpara(0,0)+ K(3,0)*new_avgpara(1,0) + K(3,1)*new_avgpara(2,0) + K(3,2)*new_avgpara(3,0));
