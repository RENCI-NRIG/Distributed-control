...
int main(int argc, char *argv[])
{
    ...
    if (argc != 9) cout<<"The command format should be ./ADMMServer <TCP_port> <# of PMUs>."<<endl;
    int ClientNum,i,j;
    ...
    int sockfd3 = 0;
    	memset(&serv_addr, '0', sizeof(serv_addr));   
    	serv_addr.sin_family = AF_INET;
    	serv_addr.sin_port = htons(atoi(argv[7])); 
   	if(inet_pton(AF_INET, argv[6], &serv_addr.sin_addr)<=0){
        	printf("\n inet_pton error occured\n");
        	return 1;
    	  } 
	if((sockfd3 = socket(AF_INET, SOCK_STREAM, 0)) < 0)   {
        ...
        if(connect(sockfd3, (struct sockaddr *)&serv_addr, sizeof(serv_addr)) == -1)   {
            perror("\n Error : Connect Failed \n");
            return 1;
        } else {
             printf("\n TCP Connection has been set up\n");
...
K<< 1.39670676256686 << 1.06457368054630 << 0.279398699553787 << -0.0724950705518260 << endr
 ...
 << -0.159125624319031 << -0.154600784389643 << 0.0125375426152317 << 0.0797297011928824 << endr;
	...
	LocalAddr.sin_port = htons(atoi(argv[8]));
   ...
   while(Iteration < SamNum){
	...
	while(CountClient < ClientNum) {
		...
		for (i=0; i<4; i++){
		        if (i==0){
					...
					for (CountClient = 0; CountClient < 1; CountClient++){
        		...
        		if (i==1){
					...
					pht = strtok(NULL, " ");
					pht = strtok(NULL, " ");
					pht = strtok(NULL, " ");					new_avgpara(i,0) = strtod(pht, NULL);
					cout<<"new_avgpara 2 ("<<i<<") is "<<new_avgpara(i,0)<<endl; 
        		}

        		if (i==2){
        			avgBufferLen = read(sockfd3, avgBuffer, DEFAULT_MAX_BUFFER_LEN);
					pht = strtok(avgBuffer, " ");
					pht = strtok(NULL, " ");
					pht = strtok(NULL, " ");
					pht = strtok(NULL, " ");					new_avgpara(i,0) = strtod(pht, NULL);
        		...
        		if (i==3){
        			avgBufferLen = read(Client_sockfd[i-3], avgBuffer, DEFAULT_MAX_BUFFER_LEN);
					pht = strtok(avgBuffer, " ");
					pht = strtok(NULL, " ");
					pht = strtok(NULL, " ");
					pht = strtok(NULL, " ");					new_avgpara(i,0) = strtod(pht, NULL);
	...
	new_u(Iteration,0)=-(K(2,2)*new_avgpara(0,0)+ K(2,0)*new_avgpara(1,0) + K(2,1)*new_avgpara(2,0) + K(2,3)*new_avgpara(3,0));
