...
int main(int argc, char *argv[])
{
    ...
    if (argc != 5) cout<<"The command format should be ./ADMMServer <TCP_port> <# of PMUs>."<<endl;
    int ClientNum,i,j;
    ClientNum = atoi(argv[1]);
	struct timeval Start, Result, t34_start,SendingTime, t34_end, t34_result, Tstart, Tend, Tresult;
	...
	timing_file << fixed << "t12	" << "Ts-t12	" << "Ts	"<<"t41"<<endl;
...
K<< 1.39670676256686 << 1.06457368054630 << 0.279398699553787 << -0.0724950705518260 << endr
 ...
 << -0.159125624319031 << -0.154600784389643 << 0.0125375426152317 << 0.0797297011928824 << endr;
   ...
   while(Iteration < SamNum){
	...
	while(CountClient < ClientNum) {
		...
		for (i=0; i<4; i++){
		        if (i==0){
					...
					for (CountClient = 0; CountClient < 3; CountClient++){
        		...
        		if (i==1){
        			avgBufferLen = read(Client_sockfd[i-1], avgBuffer, DEFAULT_MAX_BUFFER_LEN);
        			// printf("Buffer message with size %d is \n %s", avgBufferLen, avgBuffer);
					pht = strtok(avgBuffer, " ");
					pht = strtok(NULL, " ");
					pht = strtok(NULL, " ");
	...
	new_u(Iteration,0)=-(K(0,0)*new_avgpara(0,0)+ K(0,1)*new_avgpara(1,0) + K(0,2)*new_avgpara(2,0) + K(0,3)*new_avgpara(3,0));
